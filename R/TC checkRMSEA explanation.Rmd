...
4. Extract chi2 and RMSEA results as a whole sentence from Loc. Possible notations of chi2 that this code can retrieve are:
...
7. Change "v2 = 1:34" to "v2 = 1.34"; remove irrelevant info to make it easier to get the right chi2 value. E.g., "w2 (df = 24, N = 337) = 38.97" -> the program might mistakenly retrieve 24 or 337 as chi2 values. We remove every thing in the parentheses, including "v2" -> We then get "  = 38.97".
...
14. In case that the number of RMSEA retrieved and the number of chi2 retrieve are not the same (due to having a longer report and RMSEA value is too far apart from the RMSEA keyword), we try to collect the RMSEA again with more characters allowed in the string.
...
I also refer to the comparison sign retrieved from RMSEA in Step 15 and 16.
...
The minimum is 2 digis. This includes when there are more than a thousand participants, there will be "," between the second and third digits. It won't retreive a year of publication and degrees of freedom.
...
The total amount of rows will be # of chi2 values * # of Ns and # of ngroup. Here, we need to pay attention to how each value is repeated in order to have different combinations. For example, if chi2 values retrieved are 2,3,4. 
Ns retrieved are 10,20,30,40. ngroups are 8,9. (df and RMSEA always go together with chi2 because they are from the same result sentence). 
...
24. If results are not found, it would print "checkRMSEA did not find any results".
